...
test_gpu_sharing() {
    ...
    # Verify GFD labels
    replica_count=$(kubectl  get node -o json | jq '.items[0].metadata.labels["xdxct.com/gpu.replicas"]' | tr -d '"')
    if [ "$replica_count" != "10" ]; then
        echo "Required label xdxct.com/gpu.replicas is incorrect when GPU sharing is enabled - $replica_count"
        exit 1
    fi

    product_name=$(kubectl  get node -o json | jq '.items[0].metadata.labels["xdxct.com/gpu.product"]' | tr -d '"')
    if [ "$product_name" != "Tesla-T4-SHARED" ]; then
        echo "Label xdxct.com/gpu.product is incorrect when GPU sharing is enabled - $product_name"
        exit 1
    fi
